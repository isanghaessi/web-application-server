...
public class HttpRequestUtils {
	...
	private static final Pattern PATH_EXTENSION_PATTERN = Pattern.compile("\\.(html|js|css)$");
	...
	public static boolean isFileRequest(HttpMethod httpMethod, String path) {
		return HttpMethod.GET.equals(httpMethod) && PATH_EXTENSION_PATTERN.matcher(path).find();
	}
	...
	public static HttpRequest parse(InputStream inputStream) throws IOException, IllegalAccessException {
		...
		if (tokens.length < 2) {
			throw new IllegalArgumentException(java.lang.String.format("RequestHandler.parse - 요청의 헤더 첫줄이 이상합니다. firstLine: {%s}", firstLine));
		}

		String httpRequestMethod = tokens[HTTP_METHOD_INDEX];
		if (HttpMethod.getByMethod(httpRequestMethod) == null) {
			throw new IllegalAccessException(java.lang.String.format("RequestHandler.parse - 지원하지 않는 메소드입니다. method: {%s}", httpRequestMethod));
		}
		...
		if (path == null || path.isEmpty() || NO_FILE_NAME.equals(path)) {
			httpRequest.setPath(DEFAULT_PATH);
		} else {
			httpRequest.setPath(path);
		}

		StringBuilder restRequestBuilder = new StringBuilder();
		String currentLine;
		while ((currentLine = bufferedReader.readLine()) != null && !currentLine.isEmpty()) {
			// TODO(쓰레드 여러개 불림, join)
			log.debug("currentLine = " + currentLine);
			restRequestBuilder.append(currentLine);
		}
		String restRequest = restRequestBuilder.toString();
		if (!REQUEST_BODY_PATTERN.matcher(restRequest).find()) {
			httpRequest.setBody("");
		} else {
			httpRequest.setBody(REQUEST_BODY_PATTERN.matcher(restRequest).group(REQUEST_BODY_PATTERN_GROUP_INDEX));
		}
	...
	public static byte[] getFile(String fileName) throws IOException {
		...
		if (!file.exists() || !file.canRead() || file.isDirectory()) {
			throw new IllegalArgumentException(java.lang.String.format("RequestHandler.getFile - 해당 파일이 존재하지 않거나 접근할 수 없습니다. fileName: {%s}", fileName));
		}
	...
	public static void responseHeader(DataOutputStream dos, HttpStatus httpStatus) throws IOException {
		dos.writeBytes(java.lang.String.format("HTTP/1.1 {%d} \r\n", httpStatus.getCode()));
	}
